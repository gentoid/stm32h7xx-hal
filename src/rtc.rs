...
#[cfg(feature = "rm0455")]
use crate::stm32::{EXTI, RCC, RTC, TAMP};
...
impl Rtc {
    ...
    pub fn open_or_init(
        ...
        match Rtc::try_open(regs, prec, clock_source, clocks) {
            Ok(rtc) => rtc,
            Err((regs, prec, _err)) => {
                Rtc::init(regs, prec, clock_source, clocks)
            }
        }
    }
    ...
    pub fn try_open(
        ...
        clock_source: RtcClock,
        clocks: &CoreClocks,
    ) -> Result<Self, (Regs, backup::Rtc, InitError)> {
        if !prec.is_enabled() {
            return Err((regs, prec, InitError::RtcNotRunning));
        }
        ...
        if !clock_source_matches {
            return Err((regs, prec, InitError::ConfigMismatch));
        }
        ...
        if !clock_source_running {
            return Err((regs, prec, InitError::ClockNotRunning));
        }

        #[cfg(not(feature = "rm0455"))]
        return Ok(Rtc { reg: regs.rtc, prec });
        #[cfg(feature = "rm0455")]
        return Ok(Rtc { reg: regs.rtc, tamp: regs.tamp, prec });
    }
    ...
    pub fn init(
        ...
        let ker_ck = match clock_source {
            ...
            RtcClock::Hse { divider } => {
                ...
                rcc.cfgr().modify(|_, w| unsafe { w.rtcpre().bits(divider) });
        ...
        regs.rtc.wpr().write(|w| unsafe { w.bits(0xCA) });
        regs.rtc.wpr().write(|w| unsafe { w.bits(0x53) });

        // Enter initialization mode
        #[cfg(not(feature = "rm0455"))]
        let isr = regs.rtc.isr();
        #[cfg(feature = "rm0455")]
        let isr = regs.rtc.icsr();

        isr.modify(|_, w| w.init().set_bit());
        while isr.read().initf().bit_is_clear() {}

        // Enable Shadow Register Bypass
        regs.rtc.cr().modify(|_, w| w.bypshad().set_bit());
        ...
        isr.modify(|_, w| w.init().clear_bit());

        #[cfg(not(feature = "rm0455"))]
        return Rtc { reg: regs.rtc, prec };
        #[cfg(feature = "rm0455")]
        return Rtc { reg: regs.rtc, tamp: regs.tamp, prec };
    }
    ...
    pub fn read_backup_reg(&self, reg: u8) -> u32 {
        #[cfg(not(feature = "rm0455"))]
        let value = self.reg.bkpr(reg as usize).read().bkp().bits();
        #[cfg(feature = "rm0455")]
        let value = self.tamp.bkpr(reg as usize).read().bkp().bits();
    ...
    pub fn write_backup_reg(&mut self, reg: u8, value: u32) {
        #[cfg(not(feature = "rm0455"))]
        self.reg.bkpr(reg as usize).write(|w| unsafe { w.bkp().bits(value) });
        #[cfg(feature = "rm0455")]
        self.tamp.bkpr(reg as usize).write(|w| unsafe { w.bkp().bits(value) });
    }
    ...
    pub fn set_date_time(&mut self, date_time: NaiveDateTime) {
        ...
        #[cfg(not(feature = "rm0455"))]
        let isr = self.reg.isr();
        #[cfg(feature = "rm0455")]
        let isr = self.reg.icsr();
    ...
    pub fn enable_wakeup(&mut self, interval: u32) {
        ...
        #[cfg(feature = "rm0455")]
        {
            self.reg.scr().write(|w| w.cwutf().set_bit());
            while self.reg.sr().read().wutf().bit_is_clear() {}
        }
    ...
    pub fn enable_vbat_timestamp(&mut self) {
        ...
        #[cfg(not(feature = "rm0455"))]
        self.reg.isr().modify(|_, w| w.tsf().clear_bit()); // @todo should be ITSF cleared as well?
        #[cfg(feature = "rm0455")]
        self.reg.scr().write(|w| w.ctsf().clear_bit().citsf().clear_bit());
    ...
    pub fn read_timestamp(&self) -> Option<NaiveDateTime> {
        #[cfg(not(feature = "rm0455"))]
        let tsf_is_clear = self.reg.isr().read().tsf().bit_is_clear();
        #[cfg(feature = "rm0455")]
        let tsf_is_clear = self.reg.sr().read().tsf().bit_is_clear();
    ...
    pub fn is_pending(&self, event: Event) -> bool {
        ...
        match event {
            ...
            Event::AlarmA => isr.read().alraf().bit_is_set(),
            Event::AlarmB => isr.read().alrbf().bit_is_set(),
            Event::Wakeup => isr.read().wutf().bit_is_set(),
            Event::Timestamp => isr.read().tsf().bit_is_set(),
        }
    }
    ...
    pub fn unpend(&mut self, exti: &mut EXTI, event: Event) {
        ...
        match event {
            ...
            Event::AlarmA => {
                ...
                #[cfg(feature = "rm0455")]
                self.reg.scr().write(|w| w.calraf().set_bit());
            ...
            Event::AlarmB => {
                ...
                #[cfg(feature = "rm0455")]
                self.reg.scr().write(|w| w.calrbf().set_bit());
                exti.unpend(ExtiEvent::RTC_ALARM);
            }
            Event::Wakeup => {
                #[cfg(not(feature = "rm0455"))]
            ...
            Event::Timestamp => {
                ...
                self.reg.isr().modify(|_, w| w.tsf().clear_bit());
                #[cfg(feature = "rm0455")]
                // @todo Bit 3 - Clear timestamp flag Writing 1 in this bit clears 
                // the TSOVF bit in the RTC_SR register. If ITSF flag is set, TSF 
                // must be cleared together with ITSF by setting CRSF and CITSF.
                self.reg.scr().write(|w| w.ctsf().clear_bit().citsf().clear_bit());
                exti.unpend(ExtiEvent::RTC_OTHER);
            }
        }
