...
impl Crc {
    ...
    pub fn set_config(&mut self, config: &Config) {
        ...
        self.reg.pol().write(|w| unsafe { w.pol().bits(config.poly.pol()) });
        self.reg.init().write(|w| unsafe { w.init().bits(config.initial) });
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        if let Some(half_word) = half_word.next() {
            ...
            self.reg.dr16().write(|w| unsafe { w.dr16().bits(half_word) });
        }
